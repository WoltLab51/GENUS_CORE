...
This decision was planning-only before v0.4.0 finalization. It added
documentation and tests only at that time. The accepted v0.4.0 release metadata
is governed by Decision 0039, while runtime source packages, classes,
functions, exports, CLI commands, SQLite tables, sentence types, and
`SCHEMA_VERSION` remain unchanged by the spec finalization.
